...
/* =====================================================
   ...
   PKCE: стабилен финализатор (?code= ИЛИ #code=) + fallback DEMO
   ===================================================== */

const SUPABASE_URL =
  import.meta.env.VITE_SUPABASE_URL || "https://gazaegcwedqiyaefkgsr.supabase.co";
...
}: { visible: boolean; onDislike: () => void; onMessage: () => void; onLike: () => void; }) {
  if (!visible) return null;
  return (
    <div className="fixed left-1/2 -translate-x-1/2 pointer-events-auto z-[2147483647]
...
function DockBtn({ children, label, className = "", onClick }:{
  children: React.ReactNode; label: string; className?: string; onClick?: () => void;
}) {
  return (
    <motion.button type="button" whileHover={{ y: -4, scale: 1.05 }} whileTap={{ scale: 0.95 }}
      onClick={onClick}
      className={cn("size-[var(--btn)] flex items-center justify-center rounded-full shadow-2xl focus:outline-none focus-visible:ring-4 focus-visible:ring-white/60 focus-visible:ring-offset-2 focus-visible:ring-offset-white/60", className)}
      aria-label={label}
    >
...
function SwipeCard({
  ...
  return (
    <motion.div className="relative h-[68vh] min-h-[440px] rounded-[28px] overflow-hidden shadow-2xl border bg-neutral-900"
      style={{ x, rotate }} drag="x" dragConstraints={{ left: 0, right: 0 }} dragElastic={0.8}
      onDragEnd={(e, info) => {
        const t = 120;
...
}: { queue: any[]; like: (u: any) => void; nope: (u: any) => void; message: (u: any) => void; filters: any; setFilters: (f: any) => void; }) {
  const user = queue[0];
  return (
      ...
      <div className="sticky top-0 z-10 -mx-4 px-4 pb-3 bg-gradient-to-b from-white to-transparent">
        ...
        <div className="mt-3 grid grid-cols-3 gap-2 md:max-w-lg">
          ...
          <input type="text" placeholder="Град" value={filters.city || ""}
                 onChange={e => setFilters((f: any) => ({ ...f, city: e.target.value }))} className="px-3 py-2 rounded-xl border text-sm" />
      ...
      <div ref={scRef} className="mt-3 h-[65vh] rounded-3xl border bg-white overflow-y-auto p-3 space-y-3">
        {messages.map((m: any) => (
          <div key={m.id} className={cn("max-w-[82%]", m.from_id === me.id ? "ml-auto" : "")}>
            ...
            <div className={cn("px-3 py-2 rounded-2xl shadow-[0_8px_24px_rgba(0,0,0,0.08)]",
...
/* =========== Планове / монети (демо) =========== */
function Plans({ coins, setCoins, plan, setPlan }:{
  coins: number; setCoins: (fn: any) => void; plan: string; setPlan: (p: string) => void;
}) {
  const packs = [{ amt: 50, price: "4.99 лв" },{ amt: 100, price: "7.99 лв" },{ amt: 150, price: "10.99 лв" }];
  const subs = [
        ...
        {tabs.map(t => {
          ...
          return (
            <button key={t.k} onClick={() => setTab(t.k)}
              className={cn("flex-1 flex flex-col items-center gap-1 py-1.5 rounded-xl", active && "bg-neutral-900 text-white")}>
...
function TopTabs({ tab, setTab, coins, plan }:{ tab: string; setTab: (t: string) => void; coins: number; plan: string; }) {
  const tabs = [{ k: "discover", label: "Открий" },{ k: "chat", label: "Чат" },{ k: "profile", label: "Профил" },{ k: "plans", label: "Планове" }];
  return (
    <div className="hidden md:block bg-white/70 backdrop-blur border-b border-neutral-200">
      <div className="max-w-6xl mx-auto px-4 py-2 flex items-center gap-2">
        {tabs.map(t => (
          <button key={t.k} onClick={() => setTab(t.k)}
            className={cn("px-3 py-1.5 rounded-xl text-sm", tab === t.k ? "bg-neutral-900 text-white" : "hover:bg-neutral-100")}>
            {t.label}
          </button>
        ))}
...
export default function LoveLinkMVP() {
  ...
  // --- Железен PKCE финализатор (query ИЛИ hash) + чистене на URL ---
  useEffect(() => {
    (async () => {
      try {
        const href = window.location.href;
        const hasQueryCode = /\bcode=/.test(window.location.search);
        const hasHashCode  = /\bcode=/.test(window.location.hash);
        if (hasQueryCode || hasHashCode) {
          await supabase.auth.exchangeCodeForSession(href).catch(() => {});
          // чистим и от search, и от hash
          const u = new URL(window.location.href);
          ["code","state","error","error_description"].forEach(k => u.searchParams.delete(k));
          if (u.hash.includes("code=")) {
            const h = new URLSearchParams(u.hash.replace(/^#/, ""));
            ["code","state","error","error_description"].forEach(k => h.delete(k));
  ...
  // Сесия + устойчив fetch на профил (fallback DEMO)
  useEffect(() => {
    let mounted = true;
    ...
    async function setFromSession(session: Session | null) {
      ...
      if (session?.user) {
        ...
        try {
          // maybeSingle → не хвърля ако няма ред
          ...
          if (!profile) {
            ...
            await supabase.from("profiles").insert(profile).catch(() => {});
          }
          setMe(profile);
        } catch {
          // ако таблиците липсват → DEMO профил (няма бял екран)
          setMe({
            id: user.id,
  ...
  // Зареждане на профили – безопасно (ако падне, DEMO)
  useEffect(() => {
    (async () => {
      try {
        const query = supabase.from("profiles").select("*").limit(50) as any;
        if (filters.gender) query.eq("gender", filters.gender);
        if (filters.city)   query.ilike("city", `%${filters.city}%`);
        if (filters.zodiac) query.eq("zodiac", filters.zodiac);
  ...
  if (bootErr) {
    return <div style={{ padding: 16, color: "#991b1b", background: "#fee2e2", borderRadius: 12 }}>
      Входът се върна с грешка: {bootErr}
    </div>;
  }
  ...
  return (
    <div className="min-h-screen bg-[linear-gradient(135deg,#fff6f7,#eef3ff)] text-neutral-900">
      ...
      <ActionDock visible={tab === "discover" && !!current}
